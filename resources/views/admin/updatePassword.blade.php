...
@section('content')
    ...
    <div class="pl-10 pt-8 max-lg:pl-5 max-lg:max-w-14">
        <a href="{{route('admin.profile')}}" class="inline-block py-2 pb-1 px-8 text-white text-center rounded-md no-underline hover:no-underline bg-blue-600 hover:bg-blue-700" style="line-height: 1.5;">
    ...
    <section class="xl:max-w-[1300px] 2xl:max-w-[1500px] mx-auto p-4 w-full px-16 pb-6 max-lg:px-8 max-sm:px-4">
        ...
        <div class="flex items-center justify-around max-md:flex-col max-md:gap-4 gap-8 py-4 shadow w-full rounded-xl mt-6">
            ...
            <form method="POST" action="{{route('admin.changePassword', auth()->user()->id)}}" class="grid grid-cols-1 gap-4 px-4 max-w-[600px] "> 
                @csrf
                @method('PUT')
