...
@section('content')
    ...
    <section class="xl:max-w-[1300px] 2xl:max-w-[1500px] mx-auto p-4 w-full px-8 max-lg:px-4">
        ...
        <div class="py-4 pt-6">
            <h4 class="text-2xl font-bold text-center">EDIT REQUEST</h4>
        </div>

        <div class="mt-4 py-10 w-full rounded-xl shadow px-6 max-sm:px-3">
 
            <form method="POST" action="{{route('student.updateRequest', $requestDocument->id)}}" class="grid grid-cols-2 max-md:grid-cols-1 gap-4">
                @csrf
                @method('PUT')
                <div class="p-4 flex items-center gap-4">
                    ...
                    <select name="requested_document" id="requested_document" class="py-3 px-6 rounded-md border-none w-full outline-none shadow bg-transparent">
                        ...
                        <option value="{{$document->available_documents}}" {{$requestDocument->requested_document == $document->available_documents ? 'selected' : ''}}>{{$document->available_documents}}</option>
                        @empty
                ...
                <div class="p-4 flex items-center gap-4">
                    ...
                    <input type="number" min="1" value="{{$requestDocument->copies_ctc}}" name="num-ctc" id="num-ctc" class="px-6 py-3 rounded-md w-full font-bold shadow bg-transparent border-none outline-none focus:ring-2 focus:ring-green-600">
                </div>

                <div class="p-4 flex items-center gap-4">
                    <label for="num-orig" class="font-bold text-md">No. of copies of Original Document:</label>
                    <input type="number" min="1" value="{{$requestDocument->copies_orig}}" name="num-orig" id="num-orig" class="px-6 py-3 rounded-md w-full font-bold shadow bg-transparent border-none outline-none focus:ring-2 focus:ring-green-600">
                </div>
                ...
                <div class="p-4 flex items-center gap-4">
                    ...
                    <select name="check_graduate" id="check_graduate" class="py-3 px-6 rounded-md border-none w-full outline-none shadow bg-transparent">
                        <option value="Yes" {{$requestDocument->check_graduate == "Yes" ? 'selected' : ''}}>Yes</option>
                        <option value="No"  {{$requestDocument->check_graduate == "No" ? 'selected' : ''}}>No</option>
                    </select>
                </div>
                ...
                <div class="min-[470px]:p-4 flex flex-col items-center gap-4">
                    <div class="flex max-lg:flex-col max-md:flex-row max-[470px]:flex-col  items-center gap-3">
                        ...
                        <select name="last_term" id="last_term" class="py-3 px-6 rounded-md border-none sm:max-w-[150px] w-full outline-none shadow bg-transparent">
                            <option value="1" {{$requestDocument->last_term == "1" ? 'selected' : ''}}>1st term</option>
                            <option value="2" {{$requestDocument->last_term == "2" ? 'selected' : ''}}>2nd term</option>
                            <option value="3" {{$requestDocument->last_term == "3" ? 'selected' : ''}}>3rd term</option>
                            <option value="summer" {{$requestDocument->last_term == "summer" ? 'selected' : ''}}>Summer/Midyear</option>
                        </select>
                        <input type="text" placeholder="Enter school year (2022-2023)" value="{{$requestDocument->last_school_year}}" name="last_school_year" id="school_year" class="placeholder-slate-600 px-6  py-3 w-full rounded-md shadow bg-transparent border-none outline-none focus:ring-2 focus:ring-green-600">
                    </div>
                ...
                <div class="p-4 flex items-center gap-4">
                    ...
                    <select name="check_correction" id="check_correction" class="py-3 px-6 rounded-md border-none w-full outline-none shadow bg-transparent">
                        <option value="Yes" {{$requestDocument->check_correction == "Yes" ? 'selected' : ''}}>Yes</option>
                        <option value="No"  {{$requestDocument->check_correction == "No" ? 'selected' : ''}}>No</option>
                    </select>
                </div>
                ...
                <div class="p-4 flex items-center gap-4">
                 ...
                 <div class="p-4 flex items-center gap-4 max-w-[500px] mx-auto md:col-span-2">
                     <button type="submit"  class="text-white text-md  text-center rounded-md px-10 py-3 bg-green-500 hover:bg-green-600">UPDATE REQUEST</button>
                 </div>
