 ...
 */let app_modal = function ({
  ...
  if (header) {
    ...
    console.log("Header set to: " + header);
  }

  // Set body
  if (body) {
    modal.find(".app-modal-body").html(body);
    console.log("Body set to: " + body);
  }

  // Show or hide buttons
  if (buttons) {
    modal.find(".app-modal-footer").show();
    console.log("Buttons are shown.");
  } else {
    modal.find(".app-modal-footer").hide();
    console.log("Buttons are hidden.");
  }
  ...
  if (show) {
    ...
    console.log("Modal '" + name + "' is shown with data: " + data);
  } else {
    modal.hide();
    $(".app-modal-card[data-name='" + name + "']").removeClass("app-show-modal");
    $(".app-modal-card[data-name='" + name + "']").attr("data-modal", data);
    console.log("Modal '" + name + "' is hidden.");
  }
};

// // Function to show/hide the new modal
// function toggleNewSettingsModal(show = true) {
//   const modal = document.querySelector(".app-modal[data-name='new-settings']");
//   const modalCard = modal.querySelector(".app-modal-card[data-name='new-settings']");
  
//   if (show) {
//       modal.style.display = 'block';
//       modalCard.classList.add("app-show-modal");
//   } else {
//       modal.style.display = 'none';
//       modalCard.classList.remove("app-show-modal");
//   }
// }

// // Event listener for the new settings button
// document.querySelector('.new-settings-btn').addEventListener('click', function() {
//   toggleNewSettingsModal(true);
// });

// // Event listener to close the modal when clicking 'Cancel'
// document.querySelector('.app-modal[data-name="new-settings"] .cancel').addEventListener('click', function() {
//   toggleNewSettingsModal(false);
// });
    ...
    $.ajax({
        ...
        success: (data) => {
            if (data.error) {
                ...
                if (messengerColor) {
                    document.documentElement.style.setProperty('--messenger-color', messengerColor);
                }
            }
        },
        error: () => {
